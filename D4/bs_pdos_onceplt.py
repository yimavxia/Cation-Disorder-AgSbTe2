...
def read_pdos_dat(filename, num_atoms):
    with open(filename, 'r') as f:
        lines = f.readlines()
    ...
    for line in lines[1:]:  # Skip the first line
        ...
        if len(parts) == 17:
            ...
            pdos_ag = sum(float(part) for part in parts[1:5]) / num_atoms
            pdos_sb = sum(float(part) for part in parts[5:9]) / num_atoms
            pdos_te = sum(float(part) for part in parts[9:]) / num_atoms
...
ax1.set_xticks([0.00000000, 0.08335570, 0.12503360, 0.15450420, 0.24291630, 0.31510450])
ax1.set_xticklabels([r'$\Gamma$', r'$X$', r'$W$', r'$K$', r'$\Gamma$', r'$L$'], fontsize=25)
ax1.set_xlim(0, 0.31510450)
ax1.set_ylim(0, 5)
...
plt.savefig('band_structure_pdos_norm.png', dpi=300)
plt.show()
